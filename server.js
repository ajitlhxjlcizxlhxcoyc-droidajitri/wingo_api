...
app.post('/api/admin/login', (req, res) => {
  ...
  setTimeout(() => adminTokens.delete(token), 8 * 60 * 60 * 1000); // 8 hour
...
function closePeriod(mode, forcedNumber = null) {
  ...
  if (forcedNumber !== null && forcedNumber !== undefined) {
    p.number = Number(forcedNumber);
  } else if (p.number === null) {
    p.number = Math.floor(Math.random() * 10);
  }
  p.result = p.number;
  p.status = 'CLOSED';
...
app.post('/api/auth/register', (req, res) => {
  ...
  const user = {
    uid, phone, password,
    balance: 0, exp: 0, vipLevel: 0, isBanned: false,
    refereeCode: uid, referredBy: referralCode || null,
...
app.get('/api/admin/stats', adminAuth, (req, res) => {
  ...
  const pendingWithdraws = withdrawals.filter(w => w.status === 'PROCESSING').length;
...
// 🎯 WINGO RESULT (MAIN FEATURE)
// Admin sirf number 0-9 daalega, period number auto
app.post('/api/admin/wingo/result', adminAuth, (req, res) => {
  ...
  const closed = closePeriod(mode, num);
  state.current = createNewPeriod(mode);
  ...
  res.json({
    ...
    closedPeriod,
    winningNumber: num,
    newPeriod: state.current
  });
});

// Force close without specifying (random)
...
app.post('/api/admin/create-giftcode', adminAuth, (req, res) => {
  ...
  const exists = giftCodes.find(g => g.code === code.toUpperCase());
  if (exists) return res.status(400).json({ error: 'Code exists' });
