...
// ✅ Queue — admin result set karega, period end pe apply hoga
let pendingResults = {};
...
// ==================== ✅ 6-DIGIT UNIQUE UID + INVITE CODE ====================
function genUID() {
  let uid, attempts = 0;
  do {
...
function closePeriod(mode, forcedNumber = null) {
  ...
  if (forcedNumber !== null && forcedNumber !== undefined) {
    finalNumber = Number(forcedNumber);
  } else if (pendingResults[mode] !== undefined) {
    finalNumber = Number(pendingResults[mode]);
    delete pendingResults[mode];
...
app.post('/api/auth/register', (req, res) => {
  ...
  const uid = genUID();
  const inviteCode = uid; // ✅ 6-digit invite code (same as UID for simplicity)
  ...
  const user = {
    ...
    phone,
    password,
    balance: registerBonus,       // ✅ ₹58 register bonus
    exp: 0,
    vipLevel: 0,
    isBanned: false,
...
app.post('/api/user/deposit', (req, res) => {
  ...
  deposits.push(dep);
  messages.push({ id: Date.now() + 1, uid, title: 'Deposit Submitted', message: `Your deposit of ₹${amt} is pending approval`, date: new Date().toISOString() });
...
// ==================== ADMIN: WINGO ====================
// ✅ Queue result (apply on period end, NOT instant)
app.post('/api/admin/wingo/result', adminAuth, (req, res) => {
  ...
  res.json({
    message: 'Result queued — will apply when period ends',
    period: state.current.period,
    queuedNumber: num,
    willApplyAt: state.current.endTime
  });
});
...
app.get('/api/admin/wingo/pending', adminAuth, (req, res) => res.json(pendingResults));
...
app.post('/api/admin/deposit/reject', adminAuth, (req, res) => {
  ...
  const user = users.find(u => u.uid === d.uid);
  if (user) messages.push({ id: Date.now(), uid: user.uid, title: 'Deposit Rejected', message: reason || `Your deposit of ₹${d.amount} was rejected`, date: new Date().toISOString() });
